...
def get_data(name,age,address):
    conn = psycopg2.connect(dbname="postgres", user="postgres",password="abcd",host="localhost", port="5433")
    cur = conn.cursor()
...
def display_search(row):
    listbox = Listbox(frame, width=20, height=2)
...
label = Label(frame, text="ENTER YOUR DETAILS")
...
label = Label(frame, text="Search details")
